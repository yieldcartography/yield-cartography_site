...
\begin{frame}{A seven-component identity for the quarterly index return}
  ...
  \begin{itemize}
    \item $CC$ coupon accrual, $RP$ reinvestment proceeds, $RD$ clean-price
          roll-down along an unchanged curve.
    \item $ERC$ and $TPC$: revaluation from the two legs of the fitted-curve
          decomposition \( y_t(\tau) = y^{rf}_t(\tau) + tp_t(\tau) \)
          (ACM--BRW on the LW-NSS zero curve), each revalued separately.
...
\begin{frame}{The full attribution, per year of sample}
  ...
  \begin{center}
  \begin{tikzpicture}[x=0.0225cm,y=0.54cm]
  ...
  \end{tikzpicture}
  \end{center}
  \footnotesize\color{ycmuted} Components sum to the published TBSP
  log-return, $+405$ bp per year.
\end{frame}
...
\begin{frame}{Result 2: full-sample inference on the curve-move bloc}
  ...
  \begin{itemize}
    \item $ERC+TPC$ cumulates to $-337$ bp over the sample ($-17$ bp per
          year), largely the unreversed 2022 revaluation.
    ...
    \item The tests: with $e_t = r^{TBSP}_t - r^{cash}_t$ the null
          $H_0\!: \mathbb{E}[e_t]=0$ is assessed by
          \( t = \bar e \,/\, \sqrt{\widehat{V}_{NW}/T} \) and by
          bootstrap intervals on cumulative sums, likewise for $TPC$. The
          interval width reflects the concentration of variance in a single
          regime episode.
  \end{itemize}
\end{frame}
