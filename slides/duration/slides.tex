...
\begin{frame}{Motivation}
  ...
  \begin{itemize}
    ...
    \item The empirical question is the realised counterpart: of the 4.14\%
          ...
          nineteen years, \alert{how much is attributable to term-premium
          realisation} rather than to coupon accrual, reinvestment and
          roll-down?
...
\begin{frame}{A seven-component identity for the quarterly index return}
  ...
  \begin{itemize}
    ...
    \item $Cnvx$ second-order convexity; $Res$ the closure residual. The sum
          reproduces the published index by construction: the identity is an
          attribution, not a model.
...
\begin{frame}{Result 1: the carry-and-roll bloc}
  \kicker{Cumulative attribution}
  \bignum{+413 bp/yr}{cumulative CarryRoll contribution ($CC+RP+RD$),
  $+8{,}010$ bp over the sample, against $+7{,}866$ bp delivered by the
  published index. The income bloc alone accounts for essentially the whole
  realised return.}
\end{frame}
...
\begin{frame}{Result 2: full-sample inference on the curve-move bloc}
  ...
  \begin{itemize}
    ...
    \item The tests: with $e_t = r^{TBSP}_t - r^{cash}_t$ the null is
          ...
          $TPC$ series.
    \item The reason for the wide intervals is not noise throughout the sample
          but the concentration of the variance in a single regime episode.
  \end{itemize}
\end{frame}
...
\begin{frame}{The 2021Q4--2022Q3 tightening episode}
  ...
  \begin{itemize}
    ...
    \item Treating these quarters as part of a homogeneous sample conflates a
          ...
          paper conditions on the episode explicitly rather than through
          dummies.
  \end{itemize}
\end{frame}
...
\begin{frame}{Result 3: the conditional excess return}
  ...
  \bignum{+46.5 bp/q}{mean TBSP-versus-rolling-cash excess on the 74-quarter
  sub-sample, HAC $p=0.003$, cumulative bootstrap confidence interval
  $[+1{,}217,\,+5{,}534]$ bp. The excess is positive on 74 of the 78
  quarter-ends in the full sample.}
\end{frame}
...
\begin{frame}{Interpretation}
  ...
  \small
  \begin{itemize}
    \item A positive ex-ante term premium and a realised term-premium
          contribution indistinguishable from zero are not contradictory:
          realisation risk is concentrated in rare, persistent regime
          episodes, so unconditional sample means converge slowly.
    \item A TBSP-tracking exposure is characterised more accurately as a carry
          position between the term spread and the policy rate, bearing
          low-frequency regime risk, than as a systematic harvest of the
          ex-ante premium.
...
\begin{frame}{Data and econometrics}
  ...
  \begin{itemize}
    ...
    \item \textbf{Verification.} The closure residual and the tracking gap
          against the published index are reported, not absorbed: $+20$ bp and
