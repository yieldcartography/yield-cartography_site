...
\begin{frame}{The euro-area rejection is bootstrap-fragile}
  ...
  \bignum{75 $\rightarrow$ 4}{euro-area PEH rejections out of 90 cells under
  Newey--West versus under the wild block bootstrap. An order-of-magnitude
  collapse, and the cleanest illustration in the sample of the
  Bauer--Hamilton (2018) concern: asymptotic inference in overlapping-return
  regressions with persistent regressors is badly oversized.}
\end{frame}
...
\begin{frame}{Power: what the Polish non-rejection can and cannot mean}
  ...
  \begin{itemize}
    ...
    \item The Polish zero-out-of-ninety therefore cannot be read as support
          ...
          violation of plausible magnitude. Absence of evidence, not evidence
          of absence.
...
\begin{frame}{Subsample evidence: the non-rejection is a post-2020 phenomenon}
  ...
  \begin{itemize}
    ...
    \item Structural-break tests (sup-Wald) date the US and euro-area breaks
          ...
          coinciding with its own policy cycle rather than the global one.
  \end{itemize}
\end{frame}
...
\begin{frame}{Regime evidence: the euro-area anomaly has a habitat in time}
  ...
  \begin{itemize}
    ...
    \item Pooled cross-market equality tests cannot reject equal slopes across
          the three markets (Wald $p$ between 0.13 and 0.53): the
          PL--US--EA ranking is a point-estimate ordering, not a formally
          separated one.
    \item Together with the break geography, the cross-market differences line
          up with regime exposure rather than with structural properties of
          the curves.
  \end{itemize}
\end{frame}
...
\begin{frame}{Interpretation}
  ...
  \begin{itemize}
    ...
    \item Cross-market PEH rejection then tracks two things: which regimes a
          curve lived through, and how much power the test has on that
          sample. Neither is a structural property of the curve itself.
    \item Practical corollary for term-structure work on less-liquid markets:
          ...
          non-rejection on short samples as uninformative rather than as
