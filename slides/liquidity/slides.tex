...
\begin{frame}{Result 4: the supply gradient on the term premium}
  ...
  \end{tikzpicture}
  \end{center}
  \footnotesize\color{ycmuted} The supply block \{debt, foreign share, NBP
  share, debt/GDP, 12m issuance\} is jointly significant at 1\% for all
  term-premium maturities ($F$ = 4.3--9.5) and insignificant for the
  venue-capped aggregate BAS ($F=0.65$, $p=0.66$). Debt-to-GDP adds
  $+3.3$/$+5.4$/$+8.1$ bp at 2/5/10Y per pp. A COVID interaction roughly
  triples the 10Y debt sensitivity post-2020 ($+0.69$ bp per bn PLN). The
  tenor-increasing gradient is the observable implication of preferred-habitat
  demand curves (Vayanos--Vila 2021).
\end{frame}
...
\begin{frame}{Data and econometrics}
  ...
  \begin{itemize}
    ...
    \item \textbf{Estimation.} Bond-month and segment-month fixed-effects
          ...
          errors, lag bandwidth by the Andrews (1991) rule ($L \in [10,14]$);
          joint $F$-tests on the supply block; first-difference and
          non-linearity specifications as robustness.
