...
\begin{frame}{Motivation and contribution}
  ...
  \begin{itemize}
    \item The low-frequency liquidity-estimator literature is calibrated on
          ...
          exclusively on US Treasuries. Both remain largely untested on the
...
\begin{frame}{Hypotheses}
  ...
  \begin{itemize}
    ...
    \item \textbf{H2 (stocks versus flow).} Segment liquidity responds to
          stock variables (segment stock, residual-maturity concentration),
          while monthly gross issuance flow is uninformative conditional on
          them.
    \item \textbf{H3 (holder structure).} The post-2019 foreign buy-side
          retreat loads on Amihud, while the venue-capped quoted spread is
          unresponsive.
    ...
    \item \textbf{H5 (auction cycle).} Primary Dealers absorb auctions at a
          concession; secondary yields revert over the following week,
...
\begin{frame}{Result 1: issue size (H1)}
  ...
  \begin{itemize}
    \item Coefficient on $\log(\text{outstanding})$ for the quoted spread:
          $-3.5$ bp, $p<0.01$; on the composite $z$-score: $-0.123$,
          $p<0.01$; the zero-trading-days share carries the same sign and
          significance.
    \item The sign is uniform across all six low-frequency estimators and
          survives the macro block and the post-2020 regime controls.
    \item Economically, outstanding amount is the single most robust
          bond-level covariate of liquidity in the panel, consistent with the
          H1 prediction and with the deep-market evidence on issue size.
  \end{itemize}
\end{frame}

% 6 ---------------------------------------------------------------------------
\begin{frame}{Result 2: supply operates through stock variables (H2)}
  ...
  \begin{itemize}
    ...
    \item The supply margins priced into segment liquidity are therefore the
          level and the maturity composition of the outstanding stock; gross
          issuance flow is statistically irrelevant once stocks enter.
  \end{itemize}
\end{frame}
...
\begin{frame}{Result 4: the supply gradient on the term premium}
  ...
  controls, $n=167$:
  \begin{center}
  \begin{tikzpicture}[x=1.3cm,y=0.078cm]
  ...
  for all term-premium maturities ($F$ = 4.3--9.5), insignificant for the
  venue-capped BAS ($F=0.65$). Debt-to-GDP adds $+3.3$/$+5.4$/$+8.1$ bp at
  2/5/10Y per pp; a COVID interaction triples the 10Y debt sensitivity
  post-2020. The tenor-increasing gradient is the preferred-habitat
  implication of Vayanos--Vila (2021).
\end{frame}

% 9 ---------------------------------------------------------------------------
\begin{frame}{Result 5: a mark-to-market counterfactual}
  \kicker{Three-curve present-value scenario, valuation date 2026-04-21}
  \small The outstanding fixed-cash-flow universe (951.9 bn PLN face; PS/DS/WS
  fixed-coupon plus OK zero-coupon issues) is repriced under three zero
  curves, discrete annual compounding, act/365.25:
  \begin{center}
  \small
  \begin{tabular}{llrr}
    & curve & PV (bn PLN) & vs (A)\\
    \hline
    (A) & contemporaneous NSS & 949.8 & ---\\
    (B) & BRW risk-neutral ($y^{rf}$ leg) & 997.2 & $+4.99\%$\\
    (C) & NSS net of the supply wedge & 972.7 & $+2.41\%$\\
  \end{tabular}
  \end{center}
  \vspace{1mm}
  The wedge in (C) applies the Result-4 coefficients (23/81/92 bp at 2/5/10Y)
  to the 400 bn PLN debt rise of 2019--2025. The $+22.9$ bn PLN difference
  between (A) and (C) is the cumulated supply-attributable term-premium
  component of present value. First-order calculation; convexity effects are
  second order at these magnitudes.
\end{frame}

% 10 --------------------------------------------------------------------------
\begin{frame}{Result 6: auction-cycle reversion (H5)}
  \kicker{Event study on the yield path, pre-2022 sub-sample}
  \small Segment-mean $\Delta y_{s,k}$ around auction trade dates at
  $k=+5$ business days; the macro-confounded 2022--23 hiking cycle is
  excluded:
  \begin{center}
  ...
  \end{tikzpicture}
  \end{center}
  \footnotesize\color{ycmuted} Post-event yield drift is negative and
  increasing in maturity, consistent with an inventory risk premium; magnitudes
  match the US estimates of Lou--Yan--Chang (2013) and Sigaux (2018).
\end{frame}
...
\begin{frame}{Interpretation}
  ...
  \begin{itemize}
    \item \textbf{Measurement.} Under a binding venue quote cap, quote-based
          and volume-scaled liquidity estimators carry distinct information
          sets; their divergence identifies the buy-side retreat that the
          quote conceals.
...
\begin{frame}{Data and econometrics}
  ...
  \begin{itemize}
    ...
    \item \textbf{Estimation.} Bond-month and segment-month fixed-effects
          ...
          aggregate monthly time-series regressions with Newey--West errors,
          Andrews (1991) bandwidth ($L \in [10,14]$); joint $F$-tests on the
          supply block; first-difference robustness.
